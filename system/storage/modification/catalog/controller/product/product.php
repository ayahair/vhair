...
class ControllerProductProduct extends Controller {
	...
	public function index() {
		...
		if ($product_info) {
			...
			$category_query = $this->db->query("SELECT `category_id` FROM `".DB_PREFIX."product_to_category` WHERE product_id = '".$this->request->get['product_id']."' ORDER BY category_id DESC LIMIT 1");
			$category_id = (isset($category_query->row['category_id']))?$category_query->row['category_id']:0;
			...
			if ($c_path <> 0){
				$this->document->addLink($this->url->link('product/product', 'path=' . $c_path. '&product_id=' . $this->request->get['product_id']), 'canonical');
			}else{
			...
			$this->document->addStyle('catalog/view/javascript/jquery/datetimepicker/bootstrap-datetimepicker.min.css');
				...
				$custom_h1 = $product_info['custom_h1'];
				$custom_h2 = $product_info['custom_h2'];
				$custom_p_alt = $product_info['img_alt'];
				
				if (strlen($custom_h1) > 5){
					$data['heading_title'] = $custom_h1;
				}else{
					$data['heading_title'] = $product_info['name'];
				}
				
				$data['custom_p_h2'] = (strlen($custom_h2)>5)? $custom_h2 : '';
				$data['img_alt'] = (strlen($custom_p_alt)>5)? $custom_p_alt : $product_info['name']; 
			...
			if($this->config->get('cireviewpro_status')) {
				...
				$data['cireviewpro'] = $this->load->controller('cireviewpro/cireview');
				...
				$data['reviews'] = sprintf($this->language->get('text_reviews'), $review_total);

				//$avg_rating = $this->model_cireviewpro_cireview->getAvgRatingOfProducts($product_info['product_id']);

				//$data['rating'] = (int)$avg_rating;
				
			}
			$data['cireviewpro_status'] = $this->config->get('cireviewpro_status');
            // Ajax Live Options
