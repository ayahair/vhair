	...
	public function index() {
		...
		if (is_file(DIR_IMAGE . $this->config->get('config_icon'))) {
			$this->document->addLink($server . 'image/' . $this->config->get('config_icon'), 'icon');
				...
				#$$redirect_url = $this->model_error_errormanager->getcommonRedirect($current_url);
				#	if ($redirect_url != false){
				#		$redirect_url = urldecode($redirect_url);
				#		$this->model_error_errormanager->updateRedirecthits($current_url);
				#		$response_code = $this->model_error_errormanager->getResponse($current_url);
...
$data['store_url'] = HTTPS_SERVER;
		...
		if (isset($this->request->get['route'])) {
			if (isset($this->request->get['product_id'])) {
				...
				$class = '-' . $this->request->get['product_id'] . ' product-view';
