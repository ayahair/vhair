...
//--></script></div>
<?php if (($hb_seo_bulk_auto == 'y') and (isset($_GET['page']) == false)) {?>
				<script type="text/javascript">
				$(document).ready( function () {
				  $.ajax({
						  type: 'post',
						  url: '<?php echo htmlspecialchars_decode($linkseobulk); ?>'
					 });					 
				});
				</script>
				<?php } ?>
<?php if (($hb_tag_automatic == 1)  and (isset($_GET['page']) == false)) {?>
<script type="text/javascript">
$(document).ready( function () {
  $.ajax({
		  type: 'post',
		  url: '<?php echo htmlspecialchars_decode($linktags); ?>'
	 });
});
</script>
<?php } ?>
<?php if (($hb_sitemap_automatic == 1) and (isset($_GET['page']) == false)){?>
				<script type="text/javascript">
				$(document).ready( function () {
				  $.ajax({
						  type: 'post',
