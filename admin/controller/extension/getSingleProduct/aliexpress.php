...
function mspro_aliexpress_description($html){
		...
		// А ТЕПЕРЬ ТАК:
		if(strlen($out) < 10){
		    $res = explode('{"descriptionUrl":"' , $html);
		    if(count($res) > 1){
		        $res = explode('"' , $res[1] , 2);
		        if(count($res) > 1){
		            $out .= getUrl($res[0]);
		...
		$out = preg_replace(array("'<div style=\"margin-left:[^>]*?>.*?</div>'si"), Array(""), $out);
		// убрать ссылки из блока
		$out = preg_replace ("!<a.*?href=\"?'?([^ \"'>]+)\"?'?.*?>(.*?)</a>!is", "\\2", $out);
		// clear
		$out = str_replace(array("<p>	&nbsp; </p>" , '<p> &nbsp; </p>') ,'' , $out);
...
function mspro_aliexpress_price($html){
    //echo $html;exit;
        ...
        $res =  explode('<span id="j-sku-discount-price" class="p-price" itemprop="price">' , $html);
        if(count($res) > 1){
            $res = explode('<' , $res[1]);
            if(count($res) > 1){
                return mspro_aliexpress_prepare_price( $res[0] );
            }
        }
        
        $res = explode('totalValue: "' , $html);
        if(count($res) > 1){
            $res = explode('"' , $res[1] , 2);
            if(count($res) > 1){
                $price = str_replace(',' , '.' ,  $res[0]);
                if(substr($price , -1) == "."){
        ...
        $res = explode(',"formatedActivityPrice":"' , $html);
        if(count($res) > 1){
            $res = explode('"' , $res[1] , 2);
            if(count($res) > 1){
                $tres = explode('-' ,  $res[0]);
                $price = preg_replace("/[^0-9.]/", "",  $tres[0]);
                return (float) $price;
            }
        }
...
function mspro_aliexpress_sku($html){
       ...
       $res =  explode(',"page:id":"item_' , $html);
       if(count($res) > 1){
           $res = explode('_' , $res[1]);
           if(count($res) > 1){
               return $res[0];
...
function mspro_aliexpress_get_images($html){
        ...
        // NEW VARIATION
        $res = explode(',"imagePathList":' , $html , 2);
        if(count($res) > 1){
            $res = explode('"]' , $res[1] , 2);
            if(count($res) > 1){
...
function mspro_aliexpress_options($html){
	...
	// NEW OPTIONS PARSER
	// get prices if exists
	$skuPrices= array(); 
	$res = explode(',"skuPriceList":' , $html);
	if(count($res) > 1){
	    $res = explode('}],' , $res[1] , 2);
	    $res =  (array) json_decode($res[0] . '}]' , 1);
	    //echo '<pre>' . print_r($res , 1) . '</pre>';
	    if(count($res) > 0){
	        foreach($res as $skuPrice){
	            /*if(isset($skuPrice['skuPropIds']) && isset($skuPrice['skuVal']['actSkuCalPrice'])){
	                $skuPrices[$skuPrice['skuPropIds']] = $skuPrice['skuVal']['actSkuCalPrice'];
	            }*/
	            if(isset($skuPrice['skuPropIds']) && isset($skuPrice['skuVal']['actSkuMultiCurrencyCalPrice'])){
	                $skuPrices[$skuPrice['skuPropIds']] = $skuPrice['skuVal']['actSkuMultiCurrencyCalPrice'];
	            }
	        }
	    }
	}
	// echo '<pre>' . print_r($skuPrices , 1) . '</pre>';
	// get options
	$res = explode('"productSKUPropertyList":' , $html);
	if(count($res) > 1){
	    $res = explode(']}],' , $res[1] , 2);
	    $res =  (array) json_decode($res[0] . ']}]' , 1);
	    // echo '<pre>' . print_r($res , 1) . '</pre>';
	    if(count($res) > 0){
	        foreach($res as $posOpt){
	            if(isset($posOpt['skuPropertyName']) && isset($posOpt['skuPropertyValues']) && is_array($posOpt['skuPropertyValues']) && count($posOpt['skuPropertyValues']) > 0){
	                $OPTION = array();
	                $OPTION['name'] = $posOpt['skuPropertyName'];
	                $type = "select";
	                $originalType = "select";
	                $OPTION['required'] = true;
	                $OPTION['values'] = array();
	                foreach($posOpt['skuPropertyValues'] as $skuPropertyValue){
	                    if( (isset($skuPropertyValue['propertyValueDisplayName']) || isset($skuPropertyValue['propertyValueDefinitionName']) ) ){
	                        // name
	                        $name = isset($skuPropertyValue['propertyValueDisplayName'])?$skuPropertyValue['propertyValueDisplayName']:$skuPropertyValue['propertyValueDefinitionName'];
	                        $VALUES = array('name' =>  $name);
	                        // price
	                        $optPrice = '';
	                        if(isset($skuPropertyValue['propertyValueId']) && isset($skuPrices[$skuPropertyValue['propertyValueId']]) ){
	                            $optPrice = (float) ($skuPrices[$skuPropertyValue['propertyValueId']] - $originalPrice);
	                        }
	                        $VALUES['price'] = $optPrice;
	                        // image
	                       if( isset($skuPropertyValue['skuPropertyImagePath']) || isset($skuPropertyValue['skuPropertyImageSummPath']) ){
	                            $VALUES['image'] = isset($skuPropertyValue['skuPropertyImagePath'])?$skuPropertyValue['skuPropertyImagePath']:$skuPropertyValue['skuPropertyImageSummPath'];
	                            $originalType = 'image';
	                        }
	...
	//echo '<pre>'.print_r($out , 1).'</pre>';exit;exit;
...
function ali_options_get_price($sku , $priceBlock , $price){
	...
	if(count($res) > 1){
		$res = $res[1];
		$skuMultiCurrencyCalPrice_delimiter = 'actSkuMultiCurrencyCalPrice":"';
		if(strpos($priceBlock , $skuMultiCurrencyCalPrice_delimiter) < 1){
		    $skuMultiCurrencyCalPrice_delimiter = 'skuMultiCurrencyCalPrice":"';
		}
		$res = explode($skuMultiCurrencyCalPrice_delimiter , $res , 2);
		if(count($res) > 1){
			$res = explode('"' , $res[1] , 2);
			if(count($res) > 1){
				$res = (float) $res[0];
...
function mspro_aliexpress_attributes($html , $url){
    ...
    // need decode to utf-8
    $need_decode = false;
    if(strpos($html , 'rus-detail-page') > 0){
        $need_decode = true;
    }
    
    
    $res = explode('","props":' , $html  , 2);
    if(count($res) > 1){
        $res = explode(']},"' , $res[1] , 2);
        if(count($res) > 1){
             $res =  (array) json_decode($res[0] . ']' , 1);
             if(count($res) > 0){
                 foreach($res as $posAttr){
                     $ATTR = array();
                     $ATTR['group'] = "Specification";
                     if(isset($posAttr['attrName']) && isset($posAttr['attrValue']) ){
                         $ATTR['name'] =  $posAttr['attrName'];
                         $ATTR['value'] = $posAttr['attrValue'];
                         $out[] = $ATTR;
                     }
                 }
             }
    ...
    if(count($out) < 1){
        ...
        foreach($data as $attrGroup){
            ...
            }elseif(isset($attrGroup['div'][1]['div'][0]['#text']) &&
                 !is_array($attrGroup['div'][1]['div'][0]['#text']) &&
                 isset($attrGroup['div'][1]['div'][1]['ul'][0]['li']) &&
                 is_array($attrGroup['div'][1]['div'][1]['ul'][0]['li']) &&
                 count($attrGroup['div'][1]['div'][1]['ul'][0]['li']) > 0 ){
                foreach($attrGroup['div'][1]['div'][1]['ul'][0]['li'] as $posAttr){
                    $ATTR = array();
                    $group = trim(str_replace( array(":") , array("") , $attrGroup['div'][1]['div'][0]['#text'] ));
                    $ATTR['group'] = $need_decode == true?mspro_ali_decode($group):$group;
                    if(isset($posAttr['span'][0]['#text']) && !is_array($posAttr['span'][0]['#text']) && isset($posAttr['span'][1]['#text']) && !is_array($posAttr['span'][1]['#text'])){
                        $name = trim(str_replace( array(":") , array("") , $posAttr['span'][0]['#text'] ));
                        $value = trim(str_replace( array(":") , array("") , $posAttr['span'][1]['#text'] ));
...
function mspro_aliexpress_noMoreAvailable($html){
    if(stripos($html, 'page404.init();') > 0){
        return true;
    }
    return false;
}
