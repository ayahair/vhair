...
function getApiUrl($url){
    if(defined("SINGLE_API_KEY") && strlen(SINGLE_API_KEY) > 0){
        $ch = curl_init('https://multiscraper.com/api/geturl');
        curl_setopt($ch, CURLOPT_URL, 'https://multiscraper.com/api/geturl');
...
function checkProxy(){
    ...
    if((int) ($now - $last_time) > 20){
        ...
        $ch = curl_init('https://multiscraper.com/public/files/proxy.txt');
        curl_setopt($ch, CURLOPT_FOLLOWLOCATION, 1);
        curl_setopt($ch, CURLOPT_ENCODING , "");
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
        $result = curl_exec($ch);
...
function getSeoUrl_($name , $sku , $model){
