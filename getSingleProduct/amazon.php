...
function mspro_amazon_getUrl($url){
    $initialHTML = getUrl($url);
    $title = mspro_amazon_title($initialHTML);
    if($initialHTML && (!$title || $title == false || trim($title) == '') ){
        $initialHTML = getUrl($url , false, true, false);
        $title = mspro_amazon_title($initialHTML);
    }
    if(!$title || $title == false || trim($title) == ''){
        if(defined("SINGLE_API_KEY") && strlen(SINGLE_API_KEY) > 0){
            $initialHTML = getApiUrl($url);
        }
    }
    //echo $initialHTML;exit;
    return $initialHTML;
}
...
function mspro_amazon_ifChina($html){
    $res = explode("ue_sn='" , $html);
    if(count($res) > 1){
        $res = explode("'" , $res[1] , 2);
        if(count($res) > 1 && stripos($res[0] , 'zon.cn') > 0){
            return true;
        }
    }
    return false;
...
function mspro_amazon_description($html , $url){
    $ifChina = mspro_amazon_ifChina($html);
    $res = '';
...
<b> Date first available at Amazon' , $res);
    ...
    if($ifChina == true){
         $res = str_ireplace( array('src=https' , '.jpg>' , 'https') , array('src="http' , '.jpg" >' , 'http') , $res);
    }
    //echo $res;exit;
...
function mspro_amazon_price($html , $url){
    $indian = strpos($url , 'amazon.in') > 0?true:false;
...
function mspro_amazon_model($html){
    ...
    return mspro_amazon_asin($html);
}

function mspro_amazon_asin($html){
    $res = explode('data-asin="' , $html , 2);
    if(count($res) > 1){
        $res = explode('"' , $res[1] , 2);
        if(count($res) > 1){
            //echo trim($res[0]);exit;
            return trim($res[0]);
        }
    }
    $res = explode('{&quot;ASIN&quot;:&quot;' , $html , 2);
    if(count($res) > 1){
        $res = explode('&' ,$res[1] , 2);
        if(count($res) > 1){
            return trim($res[0]);
    ...
    $res = explode("path: '/gp/product/" , $html , 2);
    if(count($res) > 1){
        $res = explode("'" ,$res[1] , 2);
        if(count($res) > 1){
            return trim($res[0]);
    ...
    $res = explode('Detail","ASIN":"' , $html , 2);
    if(count($res) > 1){
        $res = explode('"' ,$res[1] , 2);
        if(count($res) > 1){
            return trim($res[0]);
...
function mspro_amazon_main_image($html){
        ...
        $instruction = 'img#ebooksImgBlkFront';
        $parser = new nokogiri($html);
